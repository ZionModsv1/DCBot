...
bot.on("ready", async () => {
  ...
  bot.user.setActivity("Getting offline raided xd", {type: "PLAYING"});
...
bot.on("message", async message => {
  ...
  if(cmd === `${prefix}ban`){
    ...
    if(!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send("You do no have Permission to do this!");
    if(bUser.hasPermission("MANAGE_MESSAGES")) return message.channel.send("That person can't be banned!");

    let banEmbed = new Discord.RichEmbed()
    .setDescription("🛡Discord Bans🛡")
    .setColor("#bc0000")
    .addField("Banned User", `${bUser} Was Banned By <@${message.author.id}>`)
    .addField("Reason", bReason)
    .addField("Banned In", message.channel)
    .setFooter("Executed By Daddy's Money Security!", "")
    .setTimestamp();
  ...
  if (message.content.startsWith(prefix + "serverip")) {
    message.delete();
    message.channel.send("```diff -3x:147.135.8.227:28158 ```");
  } 
